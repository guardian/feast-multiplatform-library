...
class GraphQlRobolectricIntegrationTest {
    ...
    @Test
    fun `given a live query when fetched with network only then it is persisted and can be read with cache only`() =
        runTest {
    ...
    @Test
    fun `given live data source when querying fronts by region then it returns fronts through real graphql client`() =
        runTest {
            val context = ApplicationProvider.getApplicationContext<Context>()
