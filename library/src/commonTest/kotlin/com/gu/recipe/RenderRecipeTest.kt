...
"utensilsAndApplianceIds": []
    ...
    fun `should convert terminology when measuringSystem does not equal sourceMeasuringSystem`() {
        ...
        val expectedRecipe = RecipeV3(
            ...
            ingredients = listOf(
                IngredientsList(
                    ingredientsList = listOf(
                        IngredientItem(text = "1 eggplant", convertedTerm = "eggplant")
                    )
                )
            )
        )
    ...
    fun `renderRecipeForTerminology ALL converts every section with block list into consideration and notes as well`() {
        ...
        val rendered = session.renderRecipeForTerminology(recipe, TerminologySection.ALL)
        val ingredient = rendered.ingredients?.first()?.ingredientsList?.first()
        ...
        assertEquals("1 <u>eggplant</u>", ingredient?.text)
        assertEquals("medium size eggplants", ingredient?.template)
        assertEquals("This is uk guidance notes for aubergine", ingredient?.ukGuidance)
        assertEquals("This is us guidance notes for eggplant", ingredient?.usGuidance)
